...
var Utils = {
    ...
    cleanText: function(str) {
        if (!str) return "";
        // 清理各种特殊符号，包括中文标点、英文标点和其他可能的干扰字符
        // 将 - 放在正则表达式开头，避免被解析为范围符号
    ...
    // 安全切换队伍（兼容不同BGI版本）
    // 如果 genshin.switchParty 不存在则跳过
    switchPartySafe: async function(teamName) {
        log.info(`switchPartySafe 被调用，队伍名称: "${teamName}"`);
        ...
        if (typeof genshin === 'undefined') {
            log.warn("genshin 对象未定义，无法切换队伍");
            return false;
        }
        
        // 检查 genshin.switchParty 是否存在
        if (typeof genshin.switchParty !== 'function') {
            log.warn(`当前BGI版本不支持 genshin.switchParty API，请手动切换到 "${teamName}" 队伍`);
            log.info(`genshin 对象类型: ${typeof genshin}`);
            log.info(`genshin.switchParty 类型: ${typeof genshin.switchParty}`);
            await sleep(2000);
            return false;
        }
    ...
    maskUid: function(uid) {
        ...
        if (uid.length <= 6) {
            return uid; // UID太短，不需要掩码
        }
        const prefix = uid.substring(0, 3);
        const suffix = uid.substring(uid.length - 3);
        const middle = '*'.repeat(uid.length - 6);
