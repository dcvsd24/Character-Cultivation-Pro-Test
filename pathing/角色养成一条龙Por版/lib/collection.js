...
var Collection = {
    ...
    getCurrentAccountUid: async function() {
        ...
        try {
            await executeCheckWithRetry(async () => {
                ...
                const maskedUid = Utils.maskUid(checkResult.accountUid);
                log.info(`✅ 当前账号UID：${maskedUid}`);
    ...
    checkScriptCooldown: function(scriptPath, cooldown, cooldownRecord, currentUid) {
        ...
        if (elapsed < cooldown) {
            ...
            log.info(`[${maskedUid}] 脚本冷却中：${scriptPath.split('/').pop()}，已过${(elapsed/3600000).toFixed(1)}小时，还需${(remaining/3600000).toFixed(1)}小时`);
            return false;
        }
        return true;
    },
    ...
    updateScriptCooldown: function(scriptPath, cooldownRecord, currentUid) {
        ...
        const maskedUid = Utils.maskUid(currentUid);
        log.info(`[${maskedUid}] 脚本冷却记录已更新: ${scriptPath.split('/').pop()}`);
    },
    
    // 清理过期冷却记录
    cleanExpiredCooldownRecords: function(cooldownRecord, currentUid, cooldown) {
        ...
        if (cleanedCount > 0) {
            file.writeTextSync(Constants.SCRIPT_COOLDOWN_RECORD, JSON.stringify(cooldownRecord, null, 2));
            const maskedUid = Utils.maskUid(currentUid);
            log.info(`[${maskedUid}] 已清理${cleanedCount}条过期冷却记录`);
        }
    },
    ...
    filterScriptsByCooldown: function(scriptList, cooldown, cooldownRecord, currentUid) {
        ...
        if (coolingScripts.length > 0) {
            const maskedUid = Utils.maskUid(currentUid);
            log.info(`[${maskedUid}] ${coolingScripts.length}个脚本在冷却中，已跳过：${coolingScripts.join(', ')}`);
        }
        
        const maskedUid = Utils.maskUid(currentUid);
        log.info(`[${maskedUid}] 脚本过滤结果：共${scriptList.length}个，可执行${availableScripts.length}个，冷却中${coolingScripts.length}个`);
    ...
    executeScripts: async function(scriptList, startIndex, maxCount, currentUid, cooldown, cooldownRecord) {
        ...
        for (let i = startIndex; i < endIndex; i++) {
            ...
            try {
                ...
                const uidRecordPath = `${Constants.EXECUTE_RECORD_PATH}_${currentUid}.txt`;
    ...
    // 获取起始索引
    getStartIndex: async function(scriptList, currentUid) {
        const uidRecordPath = `${Constants.EXECUTE_RECORD_PATH}_${currentUid}.txt`;
        let startIndex = 0;
