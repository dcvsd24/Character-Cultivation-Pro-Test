            ...
            while (Date.now() - start < timeout && count < max) {
                try {
                    ...
                    for (const res of resList) {
                        ...
                        // 清理特殊符号，提高识别准确率
                        const cleanedText = Utils.cleanText(corrText);
                        const recogName = aliasMap[cleanedText] || cleanedText || Utils.fuzzyMatch(cleanedText, Object.values(aliasMap));
                        // 记录原始和清理后的文本
                        if (corrText !== cleanedText) {
                            log.info(`清理特殊符号：[${corrText}] → [${cleanedText}]`);
                        }
