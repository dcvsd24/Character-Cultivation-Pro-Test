...
var WikiFetcher = {
    ...
    async getCharacterMaterialsFast(characterName) {
        try {
            ...
            log.info(`✅ Wiki 材料名称获取完成: Boss材料=${materialNames.bossMaterial}, 天赋怪物材料=${materialNames.talentMobMaterial}, 区域特产=${materialNames.specialty}, 天赋书=${talentBookName}`);
    ...
    async getCharacterMaterials(characterName) {
        try {
            ...
            if (fastResult.talentMobMaterialName) {
                talentMobName = await this.getTalentMobNameFromMaterial(fastResult.talentMobMaterialName);
            }
    ...
    async getWeaponInfoFast(weaponName) {
        try {
            ...
            const result = {
                ...
                weapons1MaterialName: materialNames?.weapons1Material,  // 武器1材料名称（不获取魔物名称）
                weapons2MaterialName: materialNames?.weapons2Material   // 武器2材料名称（不获取魔物名称）
            };
            
            log.info(`✅ 武器材料名称获取完成: 星级=${starLevel}, 秘境=${weaponDomainName}, 武器材料1=${materialNames?.weapons1Material}, 武器材料2=${materialNames?.weapons2Material}`);
    ...
    async getWeaponInfo(weaponName) {
        try {
            ...
            log.info(`✅ 武器信息获取完成: 星级=${starLevel}, 秘境=${weaponDomainName}, 武器魔物1=${weapons1MobName}, 武器魔物2=${weapons2MobName}`);
