...
var Character = {
                ...
                if (levelText1) {
                    ...
                    if (weaponLevel) {
                        ...
                        if (recognitionType === "all") {
                            ...
                            await sleep(800);
                            weaponDomainName = await this.recognizeDomainName("domain");
                            if (weaponDomainName) {
    ...
    recognizeDomainName: async function(materialType = "domain") {
        try {
            ...
            let domainName = "";
            let clickOffset = 0;
            let isTalentOrWeaponDomain = false;
            
            const ownershipTextList = await Utils.retryTaskWithTimeout(async () => {
                const ocr = RecognitionObject.ocr(742, 834, 398, 196);
                const capture = captureGameRegion();
                const resList = capture.findMulti(ocr);
                const items = [];
                for (let i = 0; i < resList.Count; i++) {
            ...
            if (materialType === "domain") {
                isTalentOrWeaponDomain = true;
                const fullText = ownershipTextList.map(item => item.text).join(" ");
                if (fullText.includes("纪念品")) {
                    clickOffset = 209;
                    log.info("材料类型为秘境（纪念品），偏移量：209");
                } else {
                    clickOffset = 138;
                    log.info("材料类型为秘境，偏移量：138");
                }
            } else if (materialType === "boss") {
                clickOffset = 69;
                log.info("材料类型为Boss，偏移量：69");
            } else {
                log.warn("未知的材料类型，默认使用秘境偏移");
                isTalentOrWeaponDomain = true;
                clickOffset = 138;
            }
            ...
            if (isTalentOrWeaponDomain) {
                const discoveryTextList = await Utils.retryTaskWithTimeout(async () => {
                    const ocr = RecognitionObject.ocr(777, 505, 339, 62);
                    const capture = captureGameRegion();
                    const resList = capture.findMulti(ocr);
                    const items = [];
                    for (let i = 0; i < resList.Count; i++) {
                ...
                if (discoveryTextList && discoveryTextList.some(text => text.includes("该秘境还没有被发现"))) {
                    log.info("秘境未被发现，关闭材料详情页");
            ...
            const nameTextList = await Utils.retryTaskWithTimeout(async () => {
                const ocr = RecognitionObject.ocr(1485, 17, 340, 37);
                const capture = captureGameRegion();
                const resList = capture.findMulti(ocr);
                const items = [];
                for (let i = 0; i < resList.Count; i++) {
    ...
    queryTalentDomain: async function(talentLevels) {
        try {
            ...
            const domainName = await this.recognizeDomainName("domain");
            await sleep(1000);
            await click(656, 577);
            await sleep(1500);
            if (domainName) {
    ...
    findCharacterAndGetLevel: async function(recognitionType = "all") {
        try {
            ...
            if (recognitionType === "all" || recognitionType === "weapon" || recognitionType === "talent" || recognitionType === "break") {
            ...
            if (recognitionType === "all" || recognitionType === "talent" || recognitionType === "break") {
        ...
        try {
            ...
            if (recognitionType === "all" || recognitionType === "weapon") {
                ...
                const weaponConfigObj = {
                    "weaponStar": weaponInfo.weaponStar,
                ...
                if (weaponInfo.weaponMaterialResult.weapon1.monsterNames && weaponInfo.weaponMaterialResult.weapon1.monsterNames.length > 0) {
                    weaponInfo.weaponMaterialResult.weapon1.monsterNames.forEach((name, index) => {
                        weaponMaterialObj[`Weapons1 material${index}`] = name;
                    });
                } else {
                    weaponMaterialObj["Weapons1 material0"] = "";
                }
                
                if (weaponInfo.weaponMaterialResult.weapon2.monsterNames && weaponInfo.weaponMaterialResult.weapon2.monsterNames.length > 0) {
                    weaponInfo.weaponMaterialResult.weapon2.monsterNames.forEach((name, index) => {
                        weaponMaterialObj[`Weapons2 material${index}`] = name;
                    });
                } else {
                    weaponMaterialObj["Weapons2 material0"] = "";
                }
            ...
            if (recognitionType === "all" || recognitionType === "talent") {
                const targetTalents = (settings && settings.talentBookRequireCounts ? settings.talentBookRequireCounts : "10-10-10")
                    .split('-')
                    .map(l => isNaN(parseInt(l)) ? 10 : parseInt(l));
            ...
            if (recognitionType === "all" || recognitionType === "break") {
                ...
                breakInfo.materialData.monsterMaterials.forEach((monster, index) => {
            ...
            if (recognitionType === "all" && talentDomainName) {
                ...
                log.info(`✅ 天赋秘境名称已保存：${talentDomainName}`);
            }
            
            if (recognitionType === "all" && weaponDomainName) {
            ...
            file.writeTextSync("config.json", JSON.stringify(newConfig, null, 2));
