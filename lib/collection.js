    ...
    executeScripts: async function(scriptList, startIndex, maxCount, currentUid, cooldown, cooldownRecord) {
        ...
        for (let i = startIndex; i < endIndex; i++) {
            ...
            try {
                ...
                log.info(`✅ 脚本执行成功：${script.name}（预计获取${script.count || 0}个材料），耗时：${executionTime.toFixed(2)}秒`);
                
                // 检测是否可能卡在弹窗上（33-37秒）
                if (executionTime >= 33 && executionTime <= 37) {
                    log.warn(`⚠️ 检测到脚本执行时间异常（${executionTime.toFixed(2)}秒），可能卡在弹窗上`);
                    log.info(`📌 按下ESC退出弹窗，准备执行下一个脚本`);
                    keyPress("VK_ESCAPE");
                    await sleep(1000);
                }
