...
var Overlay = (function() {
    ...
    // 显示 UID 遮挡图片
    async function showUidMask(positionX, positionY) {
        if (!useMask) return;
        try {
            uidMaskWinId = htmlMask.show("assets/uid-mask.html", "uid-mask-overlay");
            if (uidMaskWinId) {
                await htmlMask.receive(uidMaskWinId, 5000);
                log.info("✅ UID遮挡图片已显示");
                // 设置初始位置
                updateUidMaskPosition(positionX, positionY);
            }
        } catch (e) {
    ...
    function updateUidMaskPosition(x, y) {
        if (!uidMaskWinId) return;
        try {
            if (typeof htmlMask.exists === 'function' && !htmlMask.exists(uidMaskWinId)) {
                log.warn('updateUidMaskPosition: UID遮挡窗口已不存在');
                uidMaskWinId = null;
                return;
            }
            htmlMask.send(uidMaskWinId, "/position", JSON.stringify({ x: x || 0, y: y || 0 }));
